        ...
        <header className=''>
            <nav id='Navbar' className={`bg-black text-white backdrop-blur-sm fixed z-50 w-full flex items-center justify-between p-5 h-[60px] transform duration-300 shadow-black/50 shadow-lg `}>
                ...
                <div className='relative h-full'>
                    <ul className="hidden gap-7 items-center lg:flex text-base h-full relative">
                        {links.map((link, index) => (
                            <li key={index} className="border-x px-2 h-full">
                                <Link onClick={() => handleSetActiveLink(link.href)} href={link.href} className={`relative tracking-widest filter saturate-200 hover:text-verde transform duration-200 group ${ActiveLink == link.href ?'text-verde':''}`}>
                                    {link.label}
