...
function CrearServicio() {
  ...
  return (
    <form onSubmit={onSubmit}  className="flex flex-col items-center max-sm:text-xs text-sm max-sm:gap-3 max-lg:gap-4 lg:gap-5 min-h-[500px] justify-between">
        ...
        <label htmlFor="" className="flex flex-col-reverse gap-1 w-full text-black">
            <textarea 
                ...
                {... register("descrip",{
                 ...
                 className="peer w-full h-[140px] text-base border text-black outline-none border-black focus:border-verde-rgb filter saturate-[3] transform duration-200 px-2" >
            </textarea>
            {errors.descrip && (
        ...
        <button className="bg-verde-rgb text-white saturate-200 p-2 rounded-bl-lg rounded-tr-lg hover:saturate-[3] hover:shadow-lg filter w-full text-base hover:text-black trasnform duration-300"  disabled={enviando}>{enviando ? 'Registrando...' : 'Registrar Servicio'}</button>
        {error && (
